...
for(logmn_id in 1:length(logmn_seq)){
    ...
    #               .errorhandling = 'remove') %dorng% {
      ...
      bias_1 <- sum(res_DCMSE$beta_hat) - sum(beta_true)
      ...
      results_mSMSE <- mSMSE(x, y, local_sizes, v0=rep(1, p), minIterations=4, maxIterations=6, 
                             lambda_opt = F, c_h=c_h, c_h_init=c_h_init_seq,
                             verbose = F)
      bias_4 <- results_mSMSE$beta_each_iteration[1:5] - sum(beta_true)
      ...
      res_each_rep <- c(logmn, bias_1, bias_2, bias_3, bias_4, 
...
noiseType <- "hetero"
# 
os <- "C://NYU//mSMSE//p=1//results//"
p <- 1
# 
logmn_seq <- seq(1.3, 1.9, length=13)
# 
results_summary <-  matrix(NA, ncol = 24,
                            nrow = length(logmn_seq),
                            dimnames=list(as.character(round(logmn_seq, 2)),
                                          c("bias-DCMSE","var-DCMSE","coverage.rate-DCMSE",
                                            "bias-DCSMSE","var-DCSMSE","coverage.rate-DCSMSE",
                                            "bias-pooled","var-pooled","coverage.rate-pooled",
...
beta_true <- rep(1/sqrt(p), p)
for (logmn_id in 1:length(logmn_seq)) {
   results_all <- rep(NA, 17)
   for (repID in c(1:200)) {
     load(paste(os, "p=", p, "_noise=", noiseType, "_rep=", repID, ".Rdata", sep=""))
     results_all <- rbind(results_all, results[logmn_id, ])
   }
   results_all <- results_all[-1, ]
   # results_time <- results_time[-1, ]
   results_summary[logmn_id, c(1, 4, 7, 10:14)] <- apply(results_all[ ,2:9], 2, mean)
   results_summary[logmn_id, c(2, 5, 8, 15:19)] <- apply(results_all[ ,2:9], 2, function(x) mean((x-mean(x))^2))
   results_summary[logmn_id, c(3, 6 ,9, 20:24)] <- apply(results_all[ ,10:17], 2, mean)
...
print_table <- function(results_total){
  ...
  for(name in rownames(results_total)){
    ...
    row1 <- round(c(record[10] * 100, record[15] * 1e4, record[20],
                    record[12] * 100, record[17] * 1e4, record[22],
                    record[4] * 100, record[5] * 1e4, record[6]), 2)
    row2 <- round(c(
      record[11] * 100, record[16] * 1e4, record[20],
      record[1] * 100, record[2] * 1e4, record[3],
      record[7] * 100, record[8] * 1e4, record[9]), 2)
...
plot_MSE <- function(p, noiseType, logmn_seq, result_total, legend=F){
  ...
  if(p == 1){
    ...
    result_coverage <- data.frame(result_total[ , c(6, 9, 20:23, 3)])
  ...
  plot_coverage <- ggplot(result_coverage_melted, aes(logmn, coverage.rate, group=methods, color=methods, shape=methods))+
    ...
    scale_color_manual(values=(if(p==1) c("#F8766D", "#CD9600", "#7CAE00", "#00BE67", "#00BFC4", "#00A9FF", "#C77CFF") else c("#F8766D", "#CD9600", "#7CAE00", "#00BE67", "#00BFC4", "#00A9FF"))) +
    ...
    theme(legend.title=element_blank(),
          legend.position = (if(legend) c(0.2, 0.22) else "none"),
...
# 
plot_MSE(1, "norm", seq(1.3, 1.9, length=13), results_summary[as.character(seq(1.3, 1.9, length=13)), ], legend=T)
