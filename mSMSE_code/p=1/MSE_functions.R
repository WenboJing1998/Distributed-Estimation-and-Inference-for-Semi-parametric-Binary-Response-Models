                  ...
                  verbose=F)
{
  ...
  while (continueCondition) {
    ...
    # inference on beta_old
      ...
      h_t_n <- c_h * (lambda_h * 1 / n) ^ {1 / (2 * alpha + 1)}
      h_t_m <- c_h * (1 / m) ^ {2 ^ iter / (3 * alpha)}
      h_t <- max(h_t_n, h_t_m) 
      h_delta <- c_h * (lambda_h * 1 / n) ^ { delta / (2 * alpha + 1)}
    ...
    if(lambda_opt){
      ...
      if(h_t == h_t_n & !FindLambdaStar){
        lambda_h <- (v0 %*% ViVsVi %*% matrix(v0, ncol=1)) / (2 * alpha * sum((v0 %*% ViW) ^ 2))
        FindLambdaStar <- TRUE
      }
    }
  }
