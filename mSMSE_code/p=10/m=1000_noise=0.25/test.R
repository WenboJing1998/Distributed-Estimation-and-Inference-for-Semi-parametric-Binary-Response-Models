...
results_cr <- rep(NA, 21)
results_mean <- rep(NA, 21)
results_median <- rep(NA, 21)
results_var <- rep(NA, 21)

for(logmn in c(1.6, 1.7, 1.8, 1.9, 2.0)){
  print(logmn)
  m <- 1000
  n <- floor(m ^ {logmn})
  L <- floor(n / m)
  ...
  stopCluster(cl)
  results_cr <- rbind(results_cr, apply(results[ , 1:21], 2, mean))
  results_mean <- rbind(results_mean, apply(results[ , 22:42], 2, mean))
  results_median <- rbind(results_median, apply(results[ , 22:42], 2, median))
  results_var <- rbind(results_var, apply(results[ , 22:42], 2, var))
