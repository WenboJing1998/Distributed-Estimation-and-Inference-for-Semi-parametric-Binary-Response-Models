...
DC_SMSE <- function(x, 
  ...
  if(cv){
    ...
    for(c_h in c_h_seq){
      h_n <- c_h * (lambda_h / n) ^ {1 / (2 * alpha + 1)}
      cv_score <- c()
      for(k in 1:K){
        beta_list_cv <- matrix(NA, nrow = p, ncol = L)
        test_set <- c()
        for (l in 1:L) {
        
          id_l <- (sizes_cumsum[l] + 1):(sizes_cumsum[l+1])
          m <- local_sizes[l]
         
          id_test <- rep(F, m)
          id_test[((k-1) * (m/K) + 1): (k * (m/K))] <- T
          id_train <- !id_test
          test_set <- c(test_set, id_l[id_test])
         
          res <- GradientDescent(beta_0 = beta_init,
                                 x=x[id_l[id_train], ],
                                 y=y[id_l[id_train]],
                                 h_n=h_n,
                                 maxIterations = max_iter_GD,
                                 stepsizeDecrease=stepsizeDecrease)
          beta_list_cv[ ,l] <- res$beta
        }
        beta_hat <- apply(local_sizes * t(beta_list_cv) / n, 2, sum)
        cv_score <- c(cv_score, mean(y[test_set] == sign(x[test_set, ] %*% c(1, beta_hat))))
      }
      cv_scores <- c(cv_scores, mean(cv_score))
    }
    c_h <- c_h_seq[which.max(cv_scores)]
  }
  
  if(h_option=="n"){
    h_n <- c_h * (lambda_h / n) ^ {1 / (2 * alpha + 1)}
  }else if(h_option=="m"){
    h_n <- c_h * (lambda_h / local_sizes[1]) ^ {1 / (2 * alpha + 1)}
  }else{
    h_n <- c_h * sqrt((lambda_h / local_sizes[1]) ^ {1 / (2 * alpha + 1)} * (1 / n) ^ {1 / (2 * alpha + 1)})
  }
  ...
  h_n_infer <- h_n
  h_delta <- c_h * (lambda_h / n) ^ { delta / (2 * alpha + 1)}
  ...
  if(p == 1)
  {
    ...
    Vs_hat <- (1 / h_n_infer) * mean((dkerf(xbeta / h_n_infer))^2 * (x[ ,-1])^2)
    ...
    for (i in 1:n) 
    {
      ...
      Vs_hat <- Vs_hat + (1 / (n * h_n_infer)) * ydH2[i] * (x[i, -1]) %*% t(x[i, -1])
    }
  }
  
  V_inv <- solve(V_hat)
  ViU <- V_inv %*% U_hat
  ViVsVi <- V_inv %*% Vs_hat %*% V_inv
  # lambda_star <- lambda_init
  
  se <- 1 / (sqrt(n * h_n)) * sqrt(sum(ViVsVi))
  bias <- (h_n ^ (alpha)) * sum(ViU)
  ...
  if(lambda_opt){
    ...
    lambda_h <- (sum(ViVsVi)) / (2 * alpha * (sum(ViU)) ^ 2) 
    h_n <- (lambda_h / n) ^ {1 / (2 * alpha + 1)}
                  ...
                  verbose=F)
{
  ...
  ## Compute the initial estimator beta_0
    if(all(beta_init=="SMSE")){
      ...
      for(c_h_init in c_h_init_seq){
          ...
          time_init_seq <- c(time_init_seq, initial$time)
      }
      ...
      time_initial <- time_init_seq[which.max(score_init_seq)]
  ...
  while (continueCondition) {
    ...
    # inference on beta_old
      ...
      time_each_iteration <- c(time_each_iteration, time_iter) #, max(time_each_machine), time_average, time_update)
  ...
  Convergence <- (iter <= maxIterations) | (initial_machine >= L_init)
