...
def mSMSE_hd(X, y, Z, 
  ...
  while (continueCondition):
    ...
    print(f"Error: {error[-1]} train_error={training_error[-1]}")
    iter += 1
    beta_old = np.copy(beta_new)
...
n_seq = [5000, 7500, 10000, 12500, 15000]
# m_seq = [600, 800, 1000, 1200, 1400]
s_seq = [5, 10, 15, 20, 25]
p_seq = [300, 400, 500, 600, 700]
...
n_Id = taskId % len(n_seq)
n = n_seq[n_Id]
taskId2 = int((taskId - n_Id) / len(n_seq) + 1)
s_Id = taskId2 % len(s_seq)
s = s_seq[s_Id]
taskId3 = int((taskId2 - s_Id) / len(s_seq) + 1)
p_Id = taskId3 % len(p_seq)
p = p_seq[p_Id]


# for taskId in range(125):
#     taskId = int(taskId)
#     n_Id = taskId % len(n_seq)
#     n = n_seq[n_Id]
#     taskId2 = int((taskId - n_Id) / len(n_seq) + 1)
...
for rep in range(num_rep):
    ...
    res_mSMSE = mSMSE_hd(X, y, Z, local_sizes, beta, cv=False,
                         c_h_seq=[8], lambda_seq=[0.05], 
                         maxIterations=10, errorTolerence=1e-3,
                         c_init_h=0.5, c_init_lambda=0.5)
    error_mSMSE = res_mSMSE["final_error"]

    # 3. pooled-pf
    data_pf = {"X": X, "Y": y, "Z": Z}
    h_opt, c_lambda_opt, _, _ = pf_cv(X, y, Z, h_seq=[0.75, 1.5], c_lambda_seq=[1, 2], K=5)
...
np.savetxt(f"results/L2err_n={n}_s={s}_p={p}.txt", np.array(errors))
...
data = data_generate(n, p, beta, noise_type, noise_para)
X, y, Z = data["X"], data["Y"], data["Z"]
res_mSMSE = mSMSE_hd(X, y, Z, local_sizes, beta, c_h_seq=[5, 10], lambda_seq=[0.01, 0.05])

error_mSMSE = res_mSMSE["error"][-1]
...
results_plot = []
std_plot = []
L_seq = np.arange(6, 24, 2)
# L_seq = np.arange(3, 12, 1)
for L_ in L_seq:
  n_ = L_ * m_
  errors_load = np.loadtxt(f"results/L2err_n={n_}_m={m_}_p={p_}.txt")
  results_plot.append(np.mean(errors_load, axis=0))
  std_plot.append(np.std(errors_load, axis=0))


#plt.hist(errors_load[:, 1], bins=100)
#plt.hist(errors_load[:, 0], bins=100)
...
# results_plot[-4, 1] += -0.003
# results_plot[-1, 1] += -0.003
plt.plot(n_seq, results_plot[:, 0], linewidth=lw, marker='o', linestyle='-', markersize=ms, label="Avg-pf")
plt.plot(n_seq, results_plot[:, 1], linewidth=lw, marker='s', linestyle='-', markersize=ms, label="mSMSE-hd")
plt.plot(n_seq, results_plot[:, 2], linewidth=lw, marker='^', linestyle='-', markersize=ms, label="pooled-pf")

#plt.fill_between(n_seq, results_plot[:, 0] - std_plot[:, 0], results_plot[:, 0] + std_plot[:, 0], alpha=0.5)
#plt.fill_between(n_seq, results_plot[:, 1] - std_plot[:, 1], results_plot[:, 1] + std_plot[:, 1], alpha=0.5)
#plt.fill_between(n_seq, results_plot[:, 2] - std_plot[:, 2], results_plot[:, 2] + std_plot[:, 2], alpha=0.5)

plt.legend(loc='upper right', fontsize=15)
plt.xlabel(r"total sample size $n$")
plt.ylabel("$L_2$ error")
#plt.xticks(np.arange(2400, 10400, 1600))
plt.ylim(0, 0.08)
plt.grid(True)

plt.savefig(f"EE_m={m_}.png", bbox_inches='tight', dpi=150)
plt.show()
